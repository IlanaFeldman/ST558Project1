...
With the more consequential second endpoint functions created, I can now pull all of the Berry information and filter it by the columns that I may be interested in for my statistical analysis. Since I'm removing the flavor columns, I can use `dplyr::distinct` to reduce this down from 320 to 64 rows.
...
The third endpoint will be the item categories. As before, I'll include a function/input that allows you to filter out specific columns

```{r Endpoint3}
endpoint3 <- "item-category/"
...
ItemCategory <- function(id = NULL, limit = NULL, offset = NULL, alldata = FALSE, colfilter = NULL) {
  if (is.null(id) == FALSE) {
    call <- paste0(base, endpoint3, id)
    if(!is.null(limit) | !is.null(offset)) {
      warning("Combined ID with limit/offset. Ignoring the latter.")
    }
    Data <- fromJSON(call)
    CompleteData <- cbind("pocket$name" = Data$pocket$name, Data$items)
  }
  
  else {
    call <- paste0(base, endpoint3, "?limit=", limit, "&offset=", offset)
    Data <- fromJSON(call)
    if (alldata == FALSE) {
      RefinedData <- Data$results
    }
      ...
      for(i in 1:length(Data)) {
        ...
        TempTibble <- TempTibble %>% add_column(Data[[i]][[names(Data[[i]])[2]]][2])
        CompleteData <- CompleteData %>% bind_rows(TempTibble)
      }
    }
...
Something I can do with Berry + Item Category is compare the size / smoothness / soil dryness across the berries that are baking only, effort drop, in a pinch, medicine, other, picky healing, and type protection.